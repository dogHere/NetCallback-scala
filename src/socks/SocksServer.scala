...
/**
  ...
  */
class SocksServer(val localHost:String,val localPort:Int) extends Thread{
  var running = true
  ...
  def startOneThreadServer(local:Socket): Unit ={
    ...
    new Forward(local, remote).begin
  }

  override def run() {

    ss.bind(new InetSocketAddress(localHost, localPort))
    log("bind at "+localHost+":"+localPort)
    while (running) {
      val local = ss.accept()
      log("accepted a new connection from "+local.getInetAddress.getHostName+":"+local.getPort)
      new Thread() {
        override def run() {
          log("new Thread")
